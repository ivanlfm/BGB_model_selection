...
# loads packages
	...
	library(BioGeoBEARS)
		...
		#install_github(repo="nmatzke/BioGeoBEARS")library(GenSA)
	library(FD)
	library(snow)
	library(parallel)
	library(phytools)
...
###############################
	...
	# sets working directory, logfile, number of cores to use
		...
		analysis_name <- "Sicarius_model_test"
	...
	#set this to FALSE to load results from previous runs, TRUE to run the analyses
		...
		# warning: this will set run_analysis to TRUE after the (start_model)th model, so
	...
	# these will be the names of output files (plots, result objects, etc)
		filename_vector <- c(
			...
			paste(out_name, "_07strat_15_DEC", sep=""),
			paste(out_name, "_08strat_15_DVL", sep=""),
			paste(out_name, "_09strat_15_BAL", sep=""),
			paste(out_name, "_10strat_3.5_DEC", sep=""),
			paste(out_name, "_11strat_3.5_DVL", sep=""),
			paste(out_name, "_12strat_3.5_BAL", sep="")
		)
	...
	# trfn = tree file name. BioGeoBEARS needs the complete path
		#reads the tree, prunes terminals to keep only species with geographical data, saves a copy
			unpruned_tree <- read.nexus("example_Sicarius_MCC_tree.nex")
...
##########################
	...
	# other models will be constructed as modifications of this object
		...
		BioGeoBEARS_run_object_DEC$use_optimx = TRUE    			# if FALSE, use optim() instead of optimx(); you can also use "GenSA" that is slower but searchers the parameter space more thoroughly
...
###############################################
### TIME-STRATIFIED + AREA ALLOWED ANALYSES ###
###############################################

	#template modifiers — leave this here for reference
		#BioGeoBEARS_run_object$dispersal_multipliers_fn = "dispersal_matrix.txt"
		#BioGeoBEARS_run_object$areas_allowed_fn = "areas_allowed.txt"
		#BioGeoBEARS_run_object$areas_adjacency_fn = "areas_adjacency.txt"
		#BioGeoBEARS_run_object$distsfn = "distances_matrix.txt"
		#BioGeoBEARS_run_object$dispersal_multipliers_fn = "dispersalmatrix.txt"
		#BioGeoBEARS_run_object$timesfn = "timeperiods.txt"
		
	###### Galapagos unavailable in first time slice ######
		#defines a file with areas allowed
			areas_allowed_file = "example_areaallowed.txt"
		
	###### defines and loads time slices for stratified analyses ######
		# defines file with time slices with boundary at 15 Myr
			timeperiods_fn = "example_timeperiods_15.txt"
			
		# adds time slices to each of the run objects (DEC, DIVALIKE and BAYAREALIKE)
			#stratified DEC 15 Myr
				BioGeoBEARS_run_object_DEC15Myr <- BioGeoBEARS_run_object_DEC
				BioGeoBEARS_run_object_DEC15Myr$timesfn = timeperiods_fn #file with the time periods
				BioGeoBEARS_run_object_DEC15Myr$areas_allowed_fn = areas_allowed_file
				BioGeoBEARS_run_object_DEC15Myr = readfiles_BioGeoBEARS_run(BioGeoBEARS_run_object_DEC15Myr) #loads time periods into the object
				BioGeoBEARS_run_object_DEC15Myr = section_the_tree(inputs=BioGeoBEARS_run_object_DEC15Myr, make_master_table=TRUE, plot_pieces=FALSE) # divides the tree into the specified time slices
			
				# store the run object in the BGB_inputs vector
					BGB_inputs[[model_count]] <- BioGeoBEARS_run_object_DEC15Myr
					filename_vector[[model_count]]
					check_BioGeoBEARS_run(BGB_inputs[[model_count]]) # checks the object and parameters
					model_count <- model_count+1	
			
			#stratified DIVALIKE 15Myr
				BioGeoBEARS_run_object_DVL15Myr <- BioGeoBEARS_run_object_DVL
				BioGeoBEARS_run_object_DVL15Myr$timesfn = timeperiods_fn #file with the time periods
				BioGeoBEARS_run_object_DVL15Myr$areas_allowed_fn = areas_allowed_file
				BioGeoBEARS_run_object_DVL15Myr = readfiles_BioGeoBEARS_run(BioGeoBEARS_run_object_DVL15Myr) #loads time periods into the object
				BioGeoBEARS_run_object_DVL15Myr = section_the_tree(inputs=BioGeoBEARS_run_object_DVL15Myr, make_master_table=TRUE, plot_pieces=FALSE) # divides the tree into the specified time slices
				
				# store the run object in the BGB_inputs vector
					BGB_inputs[[model_count]] <- BioGeoBEARS_run_object_DVL15Myr
					filename_vector[[model_count]]
					check_BioGeoBEARS_run(BGB_inputs[[model_count]]) # checks the object and parameters
					model_count <- model_count+1
			
			#stratified BAYAREALIKE 15Myr
				BioGeoBEARS_run_object_BAL15Myr <- BioGeoBEARS_run_object_BAL
				BioGeoBEARS_run_object_BAL15Myr$timesfn = timeperiods_fn #file with the time periods
				BioGeoBEARS_run_object_BAL15Myr$areas_allowed_fn = areas_allowed_file
				BioGeoBEARS_run_object_BAL15Myr = readfiles_BioGeoBEARS_run(BioGeoBEARS_run_object_BAL15Myr) #loads time periods into the object
				BioGeoBEARS_run_object_BAL15Myr = section_the_tree(inputs=BioGeoBEARS_run_object_BAL15Myr, make_master_table=TRUE, plot_pieces=FALSE) # divides the tree into the specified time slices
			
				# store the run object in the BGB_inputs vector
					BGB_inputs[[model_count]] <- BioGeoBEARS_run_object_BAL15Myr
					filename_vector[[model_count]]
					check_BioGeoBEARS_run(BGB_inputs[[model_count]]) # checks the object and parameters
					model_count <- model_count+1	
					
	# defines file with time slices with boundary at 3.5 Myr
			timeperiods_fn = "example_timeperiods_3.5.txt"
			
		# adds time slices to each of the run objects (DEC, DIVALIKE and BAYAREALIKE)
			#stratified DEC 3.5 Myr
				BioGeoBEARS_run_object_DEC3.5Myr <- BioGeoBEARS_run_object_DEC
				BioGeoBEARS_run_object_DEC3.5Myr$timesfn = timeperiods_fn #file with the time periods
				BioGeoBEARS_run_object_DEC3.5Myr$areas_allowed_fn = areas_allowed_file
				BioGeoBEARS_run_object_DEC3.5Myr = readfiles_BioGeoBEARS_run(BioGeoBEARS_run_object_DEC3.5Myr) #loads time periods into the object
				BioGeoBEARS_run_object_DEC3.5Myr = section_the_tree(inputs=BioGeoBEARS_run_object_DEC3.5Myr, make_master_table=TRUE, plot_pieces=FALSE) # divides the tree into the specified time slices
			
				# store the run object in the BGB_inputs vector
					BGB_inputs[[model_count]] <- BioGeoBEARS_run_object_DEC3.5Myr
					filename_vector[[model_count]]
					check_BioGeoBEARS_run(BGB_inputs[[model_count]]) # checks the object and parameters
					model_count <- model_count+1	
			
			#stratified DIVALIKE 3.5Myr
				BioGeoBEARS_run_object_DVL3.5Myr <- BioGeoBEARS_run_object_DVL
				BioGeoBEARS_run_object_DVL3.5Myr$timesfn = timeperiods_fn #file with the time periods
				BioGeoBEARS_run_object_DVL3.5Myr$areas_allowed_fn = areas_allowed_file
				BioGeoBEARS_run_object_DVL3.5Myr = readfiles_BioGeoBEARS_run(BioGeoBEARS_run_object_DVL3.5Myr) #loads time periods into the object
				BioGeoBEARS_run_object_DVL3.5Myr = section_the_tree(inputs=BioGeoBEARS_run_object_DVL3.5Myr, make_master_table=TRUE, plot_pieces=FALSE) # divides the tree into the specified time slices
				
				# store the run object in the BGB_inputs vector
					BGB_inputs[[model_count]] <- BioGeoBEARS_run_object_DVL3.5Myr
					filename_vector[[model_count]]
					check_BioGeoBEARS_run(BGB_inputs[[model_count]]) # checks the object and parameters
					model_count <- model_count+1
			
			#stratified BAYAREALIKE 3.5Myr
				BioGeoBEARS_run_object_BAL3.5Myr <- BioGeoBEARS_run_object_BAL
				BioGeoBEARS_run_object_BAL3.5Myr$timesfn = timeperiods_fn #file with the time periods
				BioGeoBEARS_run_object_BAL3.5Myr$areas_allowed_fn = areas_allowed_file
				BioGeoBEARS_run_object_BAL3.5Myr = readfiles_BioGeoBEARS_run(BioGeoBEARS_run_object_BAL3.5Myr) #loads time periods into the object
				BioGeoBEARS_run_object_BAL3.5Myr = section_the_tree(inputs=BioGeoBEARS_run_object_BAL3.5Myr, make_master_table=TRUE, plot_pieces=FALSE) # divides the tree into the specified time slices
			
				# store the run object in the BGB_inputs vector
					BGB_inputs[[model_count]] <- BioGeoBEARS_run_object_BAL3.5Myr
					filename_vector[[model_count]]
					check_BioGeoBEARS_run(BGB_inputs[[model_count]]) # checks the object and parameters
					model_count <- model_count+1					
